...
async function generatePlan(payload) {
  if (!llmApiKey || activeProvider === "local") {
    return { plan: localPlan(payload), warning: "未设置国内模型 API Key，已使用本地生成器。" };
  }
  try {
    const response = await fetch(`${llmBaseURL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是严谨的产品运营方案生成器，只返回合法 JSON，不返回 Markdown。"
          },
          {
            role: "user",
    ...
    if (!response.ok) {
      return { plan: localPlan(payload), warning: `${activeProvider} 调用失败，已切换本地生成器：${data.error?.message || response.status}` };
    }
    const raw = parseLooseJson(extractText(data));
    return { plan: normalizePlan(raw, payload, activeProvider) };
  } catch (error) {
    return { plan: localPlan(payload), warning: `${activeProvider} 调用失败，已切换本地生成器：${error.message}` };
  }
}
